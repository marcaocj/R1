...
/// <summary>
/// Controla o movimento e ações básicas dos inimigos - CORRIGIDO para Unity 6
/// </summary>
...
public class EnemyController : MonoBehaviour
{
    ...
    private void Update()
    {
        if (!movementEnabled || !enemyStats.IsAlive || isDead) return;
    ...
    {
        if (!movementEnabled || navMeshAgent == null || isDead || !navMeshAgent.enabled) return;
        
        // CORREÇÃO: Verificar se o agente está ativo e na NavMesh
    ...
    {
        if (!movementEnabled || target == null || isDead) return;
    ...
    /// <summary>
    /// Para o movimento - CORRIGIDO
    /// </summary>
    public void StopMovement()
    {
        // CORREÇÃO: Verificar se NavMeshAgent está ativo antes de manipular
        if (navMeshAgent != null && navMeshAgent.enabled && navMeshAgent.isActiveAndEnabled && navMeshAgent.isOnNavMesh)
        {
            ...
            {
                Debug.LogWarning($"Erro ao parar movimento do inimigo: {e.Message}");
            }
        }
    ...
    public void SetRunning(bool running)
    {
        ...
        if (navMeshAgent != null && navMeshAgent.enabled && enemyStats != null)
        {
    ...
    /// <summary>
    /// Habilita/desabilita movimento - CORRIGIDO
    /// </summary>
    public void SetMovementEnabled(bool enabled)
    {
        ...
        // CORREÇÃO: Só desabilitar o NavMeshAgent se não estiver morto
    ...
    public void ApplyKnockback(Vector3 force, float duration = 0.3f)
    {
        if (rb != null && !isDead)
        {
            ...
            TimeManager.Instance?.CreateTimer(duration, () => {
                if (navMeshAgent != null && enemyStats.IsAlive && !isDead)
                {
    ...
    {
        if (navMeshAgent == null || isDead || !navMeshAgent.enabled) return;
    ...
    {
        isDead = true; // ADICIONADO: Marcar como morto antes de tudo
        
        // Parar movimento quando morrer - CORRIGIDO
        ...
        if (navMeshAgent != null && navMeshAgent.enabled)
        {
            ...
            {
                Debug.LogWarning($"Erro ao desabilitar NavMeshAgent na morte: {e.Message}");
            }
        }
    ...
    public bool IsMoving => isMoving && !isDead;
    public bool IsRunning => isRunning && !isDead;
    public bool IsGrounded => isGrounded;
    public bool MovementEnabled => movementEnabled && !isDead;
    ...
    {
        if (navMeshAgent == null || isDead || !navMeshAgent.enabled) return false;
    ...
    {
        if (navMeshAgent == null || isDead || !navMeshAgent.enabled) return float.MaxValue;
